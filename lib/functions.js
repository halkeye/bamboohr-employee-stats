...
module.exports = { getConfluence: getConfluence };
...
const _locations = fs.existsSync(LOCATIONS_FILE)
  ? JSON.parse(fs.readFileSync(LOCATIONS_FILE))
  : {};
module.exports.getLocation = memoize(location => {
  ...
  return request({
    ...
    json: true,
    qs: { address: location, key: process.env.GMAPS_API_KEY }
  })
    .then(json => {
...
module.exports.getEmployeeData = id => {
  return request({
    ...
    qs: {
      fields:
        'displayName,supervisor,city,country,department,division,gender,hireDate,jobTitle,location,nickname,state,stateCode,status,supervisorId,workEmail,photoUploaded,supervisorEId'
    },
    url: `https://api.bamboohr.com/api/gateway.php/${process.env
      .BAMBOOHR_SUBDOMAIN}/v1/employees/${id}`,
    headers: { accept: 'application/json' },
    json: true,
    auth: {
      user: process.env.BAMBOOHR_API_KEY,
      pass: 'x',
...
module.exports.getEmployeesData = () => {
  return request({
    method: 'GET',
    url: `https://api.bamboohr.com/api/gateway.php/${process.env
      .BAMBOOHR_SUBDOMAIN}/v1/employees/directory`,
    headers: { accept: 'application/json' },
    json: true,
    auth: {
      user: process.env.BAMBOOHR_API_KEY,
    ...
    .filter(employee => employee.jobTitle);
};
  ...
  return Promise.resolve()
    ...
    .then(summarizedValues => {
      const promises = Promise.all(
        Object.keys(summarizedValues).map(field => {
          return Promise.all(
            Object.keys(summarizedValues[field])
              .sort(
                (a, b) =>
                  summarizedValues[field][b] - summarizedValues[field][a]
              )
              .map(valueName => {
                const ret = [
                  field,
                  valueName,
                  summarizedValues[field][valueName]
                ];
                if (field === 'location') {
                  return module.exports
                    .getLocation(valueName)
                    .then(location => {
...
module.exports.getOrCreatePage = (title, parent) => {
  const content = '';
  return getConfluence()
    .getContentByPageTitleAsync(
    ...
    .then(body => {
      if (body.size === 0) {
        ...
        return getConfluence()
          ...
          .then(() =>
            getConfluence().getContentByPageTitleAsync(
              /* space */ process.env.CONFLUENCE_SPACE,
              /* title */ title
            )
          );
...
module.exports.setConfluenceContent = (title, parent, content) => {
  return module.exports.getOrCreatePage(title, parent).then(subPage => {
    return getConfluence().putContentAsync(
      /* space */ process.env.CONFLUENCE_SPACE,
      /* id */ subPage.id,
      /* version */ subPage.version.number + 1,
      /* title */ title,
      /* content */ content
    );
  });
...
module.exports.makeContentForField = (field, summarizedValues) => {
  const headers =
    '<tr>' +
    [fields[field], 'Count']
      .concat(field === 'location' ? ['Latitude', 'Longitude'] : [])
      .map(header => {
        return `<th>${header}</th>`;
      })
      .join('') +
    ...
    .then(rows => {
      return rows
        .map(row => {
          ...
          return `<tr><td>${header}</td><td>${value}</td>${extras
            .map(extra => `<td>${extra}</td>`)
            .join('')}</tr>`;
        })
        .join('');
    })
